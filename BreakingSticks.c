...
long long CutTheSticks(long long n) {
    if (n == 1) return 1;
    unsigned int j = 0;
    for (unsigned int i = 0; i < k && n > 1;)
    {
        if (n % W[i] == 0)
        {
            F[j] = W[i];
...
long longestSequence(int a_count, long* a) {
    long long total = 0;
    InitializeSticks();
    for (int i = 0; i < a_count; i++)
    {
        total += CutTheSticks(a[i]);
    }
    return total;
}
...
int main()
{
    ...
    long result = longestSequence(n, a);
    fprintf(fptr, "%ld\n", result);
