...
int formingMagicSquare(int s_rows, int s_columns, int** s) {
  ...
  for (int i = 0; i < s_rows; i++) {
    for (int j = 0; j < s_columns; j++) {
      int k = s[i][j];
      ans[0] += abs(k - arr0[i][j]);
      ans[1] += abs(k - arr1[i][j]);
      ans[2] += abs(k - arr2[i][j]);
      ans[3] += abs(k - arr3[i][j]);
      ans[4] += abs(k - arr4[i][j]);
      ans[5] += abs(k - arr5[i][j]);
      ans[6] += abs(k - arr6[i][j]);
      ans[7] += abs(k - arr7[i][j]);
    }
    }
    int min =ans[0];
    for(int i=0;i<8;i++)
