...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    {
        int t = ranked[rank];
        if (uniqueElementCount)
        {
            if (ranked[uniqueElementCount - 1] == t)
            {
                continue;
            }
        }
        ranked[uniqueElementCount++] = t;
    }

    for(int i=0;i<player_count;i++)
    {
        int temp=uniqueElementCount;
        while(temp>0 && ranked[--temp]<=player[i] )
        {
            uniqueElementCount=temp;
        }
        result[i] = uniqueElementCount+1;
    }
    return result;
}
